...
> Source PRD: `docs/PRD.md` / GitHub Issue #1
...
Home page at `/` showing 3 balance cards (total income, total expenses, net balance), date range picker defaulting to current month, donation summary chart (by type), expense summary chart (by category), and quick action buttons (New Donation, New Expense). Fetches from `/api/v1/reports/balance`, `/api/v1/reports/donations`, `/api/v1/reports/expenses`.

### Acceptance criteria

- [ ] 3 summary cards display income, expenses, net balance for selected period
- [ ] Date range picker defaults to current month, updates all data on change
- [ ] Bar/pie chart showing donations grouped by type
- [ ] Bar/pie chart showing expenses grouped by category
- [ ] Quick action buttons navigate to `/donations/new` and `/expenses/new`
- [ ] Loading and error states shown as inline alerts
